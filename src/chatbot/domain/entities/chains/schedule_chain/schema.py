...
class ReadCommandModel(BaseModel):
    ...
    flag: bool = Field(
        default=False,
        description="Indicates whether to perform a schedule read operation.",
    )
    time_max: datetime = Field(
        default_factory=datetime.now,
        description="Upper bound for the event start time. Only events that start **before or at** this time must be included.",
    )

    time_min: datetime = Field(
        default_factory=datetime.now,
        description="Lower bound for the event start time. Only events that start **after or at** this time must be included.",
    )

...
from chatbot.domain.entities.chains.domain_selector_chain.schema import QuestionResponse
from chatbot.domain.entities.chains.general_chat_chain.general_chain import (
    general_chain,
)
from chatbot.domain.entities.chains.weather_chain.weather_chain import weather_agent

from chatbot.domain.entities.chains.domain_selector_chain.prompt import prompt
from chatbot.domain.entities.chat_models.chat_models import groq_chat, groq_deepseek
...
domain_selector_chain = prompt | groq_chat
...
print(
    multi_domain_chain.invoke(
        {"question": "3일후 스케쥴과 오늘 날씨 알려줘", "chat_history": []}
    )
)
